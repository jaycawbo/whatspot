...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (request.status !== 'accepted') return errorResponse('Request is not accepted', 409);
    if (!request.holding_expires_at || new Date(request.holding_expires_at) <= new Date()) {
    ...
    console.error('redeem-request error:', message);
