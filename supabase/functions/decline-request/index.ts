...
Deno.serve(async (req) => {
  ...
  let body: { request_id?: string; comment?: string };
  try {
    body = await req.json();
  } catch {
  ...
  if (comment !== undefined && comment.length > 200) {
    return errorResponse('comment must be 200 characters or fewer', 400);
  }
  ...
  try {
    ...
    const { error: updateError } = await serviceClient
      .from('requests')
      .update({ status: 'declined', decline_comment: comment ?? null })
      .eq('id', request_id);

    if (updateError) throw updateError;

    return jsonResponse({ request_id, comment: comment ?? null });

  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Unknown error';
    console.error('decline-request error:', message);
