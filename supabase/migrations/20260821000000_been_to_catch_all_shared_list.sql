-- "Been To" becomes a catch-all for any rated venue (liked, loved, or
-- disliked), not just rating = 'liked'. Mirrors LIST_MATCHER['Been To'] in
-- src/lib/spotListConstants.js (issue #294). A venue can now match both
-- 'Been To' and Favourites/Didn't Like It.
...
BEGIN
  ...
  WHERE uvi.user_id = v_status.user_id AND (
    ...
    (v_status.list_key = 'Been To'         AND uvi.interaction_type = 'rated') OR
